...
<?php new_page() ?>
	...
	<?php
		$fafvcalc = new FaFvCalcNEHRP2015();
	...
	<div class="mapwrapper" style="margin:5em 0 1em;">
	<?php
		print $fafvcalc->getFaFvUndeterminedTableMarkup($_data->num('ss'),
			$_data->num('s1'), $_siteclass, array(
			'title' => 'Spectral Response Acceleration Parameters',
			'caption' => 'Table 11.4&ndash;3: Site Coefficients for '.
					'Undetermined Soil Sites (excluding Class E or F),'.
					'F<sub>a</sub> and F<sub>v</sub>'
		));
	?>
	</div>
...
<?php last_page() ?>
	...
	?>
	<?php
		print $fafvcalc->getFpgaUndeterminedTableMarkup($_data->num('pga'),
			$_siteclass, array(
			'title' => 'Mapped MCE Geometric Mean Peak Ground Acceleration, PGA',
			'caption' => 'Table 11.8&ndash;2: Site Coefficients for '.
					'Undetermined Soil Sites (excluding Class E or F),'.
					'F<sub>pga</sub>'
		));
	?>
