...
<?php
  ...
  if ($SHARE) {
	...
	$url = urlencode(sprintf('https://%s%s', $_SERVER['SERVER_NAME'], $_SERVER['REQUEST_URI']));
  ...
  $services = array(
    'facebook'     => array('Facebook'      => sprintf('https://www.facebook.com/sharer.php?t=%s&amp;u=%s', $title, $url)),
    'twitter'      => array('Twitter'       => sprintf('https://twitter.com/share?text=%s%s&amp;url=%s', $title, urlencode(' (via @usgs)'), $url)),
